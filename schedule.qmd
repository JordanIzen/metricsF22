...
<ul class="icon-list">
  <li> {{< fas fa-info-circle >}} **Content** materials contains suggested readings, more details about assignments, math appendices, and other helpful resources. I suggest you view these **before** each class.</li>
...
```{r, eval = F, echo = F}
sched %>%
    select(group, ` ` = col_date, Title = col_title, Content = col_content,
...
show_table <- function(group_id) {
  # Add heading
  cat(as.character(paste("\n\n###", nested_sched$group[[group_id]], "\n\n")))
  
  # Make table
  tbl <- nested_sched$data[[group_id]] %>% 
    #select(-subgroup) %>% 
