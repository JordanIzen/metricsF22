...
- [Only new change:]{.hi-purple} divided by $n-\color{#6A5ACD}{k}-1$ due to use of $k+1$ degrees of freedom to first estimate $\beta_0$ and then all of the other $\beta$'s for the $k$ number of regressors^[Your textbook defines *k* as including the constant, the denominator would be *n-k* instead of *n-k-1*.]
...
- In the end, recall [$R^2$ was never that useful]{.hi-turquoise}^[...for measuring causal effects (our goal). It *is* useful if you care about prediction instead!], so don't worry about the formula
