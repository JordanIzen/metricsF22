...
[Tibbles & Piping](#tibbles-piping)

[Importing Data](#importing-data)

[Tidying (Pivoting/Reshaping) Data](#tidying-pivotingreshaping-data)
...
[`filter()` Select Rows by Condition](#filter-select-rows-by-condition)

[`mutate()` Create New Variables](#mutate-create-new-variables)

[`summarize()` Create Statistics](#summarize-create-statistics)

[`group_by()` Grouped Summaries](#group-by-grouped-summaries)

[`dplyr` Other Useful Commands](#dplyr-other-useful-commands)
...
- The `magrittr` package allows use of the **"pipe" operator** (`%>%`)^[Keyboard shortcuts in R Studio: `CTRL+Shift+M` (Windows) or `Cmd+Shift+M` (Mac). There is also a *new* pipe built into Base `R`: `|>`.]
...
df <- read_csv("C:/Documents/Jordan Izenwasser/Downloads/my_data.csv")
```
...
setwd("C:/Documents/Jordan Izenwasser/MyProj")
...
a <- "C:/Documents/Jordan Izenwasser/MyProj"
b <- "C:/Documents/Your Name/YourProj"
...
- We'll fix this soon with `R Projects`
    - The working directory is set to the Project Folder by default
    - Same for everyone on any computer!

## Data Import Cheat Sheet

![[R Studio: Data Import Cheat Sheet](https://raw.githubusercontent.com/rstudio/cheatsheets/main/data-import.pdf)](images/data-import-cheatsheet.png)
...
- ["tidy" data]{.hi} are (an opinionated view of) data where
  1. Each [variable]{.hi-purple} is in a [column]{.hi-purple}
  2. Each [observation]{.hi-purple} is a [row]{.hi-purple}
  3. Each [observational unit]{.hi-purple} forms a [table]{.hi-purple} (or "every value is its own cell.”)
...
![](images/tidydata1.png){fig-align="center"}
...
![](images/untidy-gapminder-hiv.png)

## Examples of Untidy Data

![](images/untidy-census.png)
...
![](images/original-dfs-tidy.png){fig-align="center" width="600"}
...
![](images/tidyr-pivoting.gif){fig-align="center" width="1400"}

[Source: Garrick Aden-Buie's [tidyexplain](https://github.com/gadenbuie/tidyexplain)]{.source}
...
- **Common source of “un-tidy” data**: [Column headers are values, not variable names!]{.hi-purple} 
...
- **Another common source of “un-tidy” data**: [observations are scattered across multiple rows!]{.hi-purple} 
...
![[R Studio: Data Tidying Cheat Sheet](https://raw.githubusercontent.com/rstudio/cheatsheets/main/tidyr.pdf)](images/data-tidying-cheatsheet.png)
...
- We can use the `join_*` family of functions from `dplyr`
  - Requires two data frames, `x` and `y`
  - Can identify variable(s) we want to join on using `by = "var_name"`
  - Joining variables must have same name in both data frames
  - If we don't specify `by= `, `dplyr` will try to find a var with the same name to join on
:::
...
ex_gdp <- tribble(
  ~"country", ~"year", ~"gdp",
  "United States", 2000, 10.25,
  "United States", 2010, 15.05,
  "Canada", 2000, .744,
  "Canada", 2010, 1.62,
  "China", 2000, 1.21,
  "China", 2010, 6.09
...
::: {.column width="50%"}
```{r}
left_join(ex_wide,ex_gdp, by = c('country','year'))
...
# `group_by()`: Grouped summaries {.centered background-color="#314f4f"}
...
# `dplyr`: Other Useful Commands {.centered background-color="#314f4f"}
...
![[R Studio: Data Wrangling Cheat Sheet](https://raw.githubusercontent.com/rstudio/cheatsheets/main/data-transformation.pdf)](images/data-wrangling-cheatsheet.png)
