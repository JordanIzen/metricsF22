...
# Omitted Variables and Bias {.centered background-color="#314f4f"}
...
-   Violates **zero conditional mean assumption** $$\mathbb{E}(u_i|X_i)\neq 0 \implies$$
...
Consider our recurring class size and test score example: $$\text{Test score}_i = \beta_0 + \beta_1 \text{STR}_i + u_i$$
:::
...
-   [Bias]{.blue} from $\%EL$ is positive
    ...
    -   Implies that our $\color{red}{\hat{\beta}_1}$ ***over*****states** the effect of reducing STR on improving Test Scores
...
::: {.column width="25%"}
![](images/3apples.jpg){width="400"} Treatment Group
:::

::: {.column width="25%"}
![](images/3apples.jpg){width="400"} Control Group
:::
:::
...
# The Multivariate Regression Model {#the-multivariate-regression-model .centered background-color="#314f4f"}
...
-   $\beta_0, \beta_1, \beta_2, \cdots , \beta_k$ are [parameters]{.hi-purple} that describe the population relationships between the variables
    ...
    -   we estimate $k+1$ parameters ("betas") on $k$ variables[^1]
...
# The Multivariate Regression Model {.centered background-color="#314f4f"}
