...
[2.1 --- Data 101 & Descriptive Statistics]{.custom-title}
...
::: columns
::: {.column width="50%"}
- We want to use econometrics to [identify]{.hi} causal relationships and make .hi[inferences] about them
...
2. [Inferential Statistics:]{.hi} infers properties about a larger population from the properties of a sample^[We'll encounter inferential statistics mainly in the context of regression later.]
...
- See [today's appendix](/resources/appendix/2.1-appendix) for more about the [summation operator, $\displaystyle\sum$]{.hi-purple}, it'll come up again!
...
1. Download the `summaries.R` [file](/files/summaries.R) from the website^[One day I'll make this part of a package I'll write.] and move it to your working directory/project folder

2. Load the function with the `source()` command:^[If it *was* a package, then you'd load with `library()`. But you can run a single `.R` script with `source()`.]
...
3) The function has at least 2 arguments: the `data.frame` (automatically piped in if you use the pipe!) and then all variables you want to summarize, separated by commas^[There is one restriction: No variable name can have an underscore `(_)` in it. You will have to rename them or else you will break the function!]
...
4) When `render`ed in `Quarto`, it looks nicer:
