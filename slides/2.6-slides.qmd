...
[2.6 --- Inference for Regression]{.custom-title}
...
[Confidence Intervals Using the `infer` Package](#confidence-intervals-using-the-infer-package)

[Confidence Intervals, Theory](#outliers)
...
## Why Sample vs. Population Matters {.smaller}

```{R, gen-dataset, include = F, cache = T}
set.seed(2)
sample_1 <- ca_school %>%
  sample_n(size = 50)
lm1 <- lm(testscr ~ str, data = sample_1)
sample_2 <- ca_school %>%
  sample_n(size = 50)
lm2 <- lm(testscr ~ str, data = sample_1)
sample_3 <- ca_school %>%
  sample_n(size = 50)
lm3 <- lm(testscr ~ str, data = sample_1)
...
#| fig-align: center
#| fig-width: 14
scatter_base <- ggplot(data = ca_school)+
  aes(x = str,
      y = testscr)+
...
<br>
$Y_i = `r round(school_reg$coefficients[1], 2)` + `r round(school_reg$coefficients[2], 2)` X_i + u_i$
...
scatter_1 <- scatter_base+
  geom_point(data = sample_1, aes(x = str, y = testscr), color = "blue")+
  geom_smooth(data = sample_1, aes(x = str, y = testscr), method = "lm", color = "black", se = F)
scatter_1
```

**Sample 1:** 50 random individuals
...
scatter_2 <- scatter_1+
  geom_point(data = sample_2, aes(x = str, y = testscr), color = "blue")+
  geom_smooth(data = sample_2, aes(x = str, y = testscr), method = "lm", color = "black", se = F)
scatter_2
```
...
scatter_3 <- scatter_2+
  geom_point(data = sample_3, aes(x = str, y = testscr), color = "blue")+
  geom_smooth(data = sample_3, aes(x = str, y = testscr), method = "lm", color = "black", se = F)
scatter_3
```
...
- This exercise is called a [(Monte Carlo) simulation]{.hi-purple}
  - I'll show you how to do this soon with the `infer` package
...
boots <- ca_school %>%
  ...
  generate(reps = 1000,
           type = "bootstrap") %>%
  nest(data = -c(replicate)) %>%
 mutate(model = map(data, ~lm(testscr ~ str, data = .x)), 
...
sim_scatter <- scatter + 
  geom_abline(data = boot_coefs, aes(intercept = intercept, slope = slope, group = replicate), color = "gray")+
  geom_smooth(method = "lm", color = "red", se = F)
sim_scatter
```
...
2. `generate()` a bootstrap distribution based on a certain number of random draws

3. `calculate()` $\hat{\beta_1}$ for each sample; create confidence intervals 
...
Results Bootstrapped from Our Sample
...
#| echo: true
ci <- boot %>%
  summarize(lower = quantile(stat, 0.025), # Value that contains 2.5% of data
            upper = quantile(stat, 0.975)) # Value that contains 97.5% of data
ci
...
R can grab the bounds of the C.I as well
```{r}
#| echo: true
ca_school %>% #<< # save this
  specify(testscr ~ str) %>%
...
# Confidence Intervals, Theory {.centered background-color="#314f4f"}
...
::: columns
::: {.column width="50%"}
- $\color{#6A5ACD}{(1- \alpha)}$ is the [confidence level]{.hi-purple} of our confidence interval
  - $\color{#6A5ACD}{\alpha}$ is the [“significance level”]{.hi-purple} that we use in hypothesis testing
  - $\color{#6A5ACD}{\alpha}=$ probability that the true parameter is *not* contained within our interval


- [Critical value]{.hi-purple} - value of a distribution used to construct C.I and conduct hypothesis testing
...
- In practice, we never know the true population sd of $\beta_1$, $\sigma_{\beta_1}$. We also may have a small sample size of $n<30$.

- If either of these are true, we can't use the Central Limit Theorem to assume that $\hat{\beta_1}$ has a standard normal distribution. 

- Instead, it has a [Student's t-distribution]{.hi-purple} with n-1 degrees of freedom

- As n grows, the t-distribution approximates a normal distribution, so in practice we will use critical values which correspond to z-scores. However, we still always use the t-distribution for C.I and hypothesis testing 

. . .

- If $\alpha=0.1$, we have a 90% confidence level and $Z_{critical} = \pm 1.645$
- If $\alpha=0.05$, we have a 95% confidence level and $Z_{critical} = \pm 1.96$
- If $\alpha=0.01$, we have a 99% confidence level and $Z_{critical} = \pm 2.58$



## Calculating C.I Using Theory
- Exactly 95% of data falls withing 1.96 standard deviations of the mean, thus we often use a critical value of 1.96
- To construct a C.I of our str coefficient we would then
$$([-2.28-(1.96)*0.4798], [-2.228 + (1.96)*0.4798]) = (-3.22,-1.33)$$
...
❌ [95% of the time, the true effect of class size on test score will be between  -3.22 and -1.33]{.red}

. . .

❌ [We are 95% confident that a randomly selected school district will have an effect of class size on test score between -3.22 and -1.33]{.red}

. . .

❌  [The effect of class size on test score is  -2.28 95% of the time.]{.red}

. . .

✅ [We are 95% confident that in similarly constructed samples, the true effect is between  -3.22 and -1.33]{.green}
