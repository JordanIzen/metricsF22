...
- i.e. $Z$ is in the error term, $u_i$
- $Z$ is part of the data generating process for $Y$
...
$$\mathbb{E}(u_i|X_i)\neq 0 \implies$$ 
  - knowing $X_i$ tells you something about $u_i$ 
...
- [Bias]{.blue} from $\%EL$ is positive
  ...
  - This is because larger classes are more likely to contain more ESL students and having more ESL students is associated with lower test scores. Thus our effect is too negative due to the omitted variable!
  - Implies that our $\color{red}{\hat{\beta}_1}$ __*over*states__ the effect of reducing STR on improving Test Scores
    - This is different than over- and underestimating 
...
::: columns
::: {.column width="50%"}
- Let's construct an ideal [random controlled trial (RCT)]{.hi-turquoise} to test the effect of str on test scores
