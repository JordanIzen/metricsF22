...
- [Standard Error of the Regression]{.hi} $\color{#e64173}{\hat{\sigma}_u}$ is an estimator of the standard deviation of $u_i$
...
In large samples (where $n-2 \approx n)$, SER $\rightarrow$ standard deviation of the residuals. This is why it is important to have large samples! Standard errors will be smaller
...
::: columns
::: {.column width="50%"}
- $\mathbf{\hat{\beta_1}}$ is the [Best Linear Unbiased Estimator (BLUE)]{.hi} estimator of  $\mathbf{\beta_1}$ **when $X$ is exogenous**^[The proof for this is known as the famous [Gauss-Markov Theorem](https://en.wikipedia.org/wiki/Gauss%E2%80%93Markov_theorem). We will prove this now.]
