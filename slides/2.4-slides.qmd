...
\color{#0047AB}{\text{Observed}_i} &= \color{#047806}{\text{Model}_i} + \color{#D7250E}{\text{Resid}_i} \\
\end{align*}
...
$$\underbrace{\color{#0047AB}{Y_i}}_{\color{#0047AB}{Observed}}=\underbrace{\color{#047806}{\hat{Y}_i}}_{\color{#047806}{Model}}+\underbrace{\color{#D7250E}{\hat{u}_i}}_{\color{#D7250E}{Resid}}$$

- Recall OLS estimators chosen to **minimize** [**Sum of Squared Residuals (SSR)**]{.red}: $\left(\displaystyle \sum^n_{i=1}\hat{u_i}^2\right)$

## $R^2$

- Primary measure is [R-squared]{.hi}, aka [coefficient of determination]{.hi}, the fraction of variation in $\color{#0047AB}{Y_i}$ explained by variation in predicted values $\color{#047806}{\hat{Y}_i}$
...
- $R^2 \in [0,1]$ and can be interpreted as the percent of the variation in $Y$ explained by variation in $X$ when there is one variable in the regression

- $R^2$ will always increase as we add more variables to the regression so we loose that interpretation when we exceed one variable. We will talk about $R^2$-adjusted later in the course, which will explain the goodness of fit better when we have multiple regressors 

## $R^2$ Formula I

$$R^2 = \frac{\color{#047806}{SSE}}{\color{#0047AB}{SST}}$$


- [**Explained Sum of Squares (SSE)**]{.green}: sum of squared deviations of *predicted* values from their mean

$$\color{#047806}{SSE} = \sum^n_{i=1}(\hat{Y_i}-\bar{Y})^2$$

. . .

- [**Total Sum of Squares (SST)**]{.blue}: sum of squared deviations of *observed* values from their mean
...
$$R^2 = \frac{SSE}{SST} = \frac{\color{purple}{C}}{\color{red}{A}+\color{purple}{C}}$$
...
$$R^2 = \frac{SSE}{SST} = \frac{\color{purple}{C}}{\color{red}{A}+\color{purple}{C}}=0.05$$
...
$$R^2 = \frac{var(\hat{Y})}{var(Y)} = \frac{\color{red}{\frac{1}{n-1}}\sum^n_{i=1}(\hat{Y_i}-\bar{Y})^2}{\color{red}{\frac{1}{n-1}}\sum^n_{i=1}(Y_i-\bar{Y})^2} \rightarrow \frac{SSE}{SST}$$

- SSE and SST are simply the numerators of the variance of $\hat{Y}$ and $Y$, respectively (i.e. before dividing by $n-1$, which will cancel out).


## Standard Error of the Regression

- Another measure of how well the model fits the data is the [standard error of the regression]{.hi} $\color{#e64173}{\hat{\sigma}_u}$, which is an estimator of the standard deviation of $u_i$. It is in the same units as dependent variable.
  - Also know as root mean squared error (root MSE in Stata) or residual standard error (R)
  - If the SSR is the variance of the residuals (in squared units of dependent variable), $\hat{\sigma}_u$ is the standard deviation of the residuals
...
In large samples (where $n-2 \approx n)$, SER $\rightarrow$ standard deviation of the residuals. This is why it is important to have large samples! Standard errors will be smaller which means we can fit the data better and make better statistical inference
...
Luckily, we generally don't need to calculate all these statistics by hand. R does it for us!
```{r}
#| echo: true
#| output-location: fragment
school_reg %>% summary()

# Scroll down to find R2
```
...
::: columns
::: {.column width="50%"}
1. [Causal inference]{.hi}: what is the effect of SNAP on poverty reduction?
...
::: columns
::: {.column width="50%"}
1. [Causal inference]{.hi}: how does the time of day impact what shows people select?
...
- [*Sampling* randomness]{.hi-purple}: different samples will generate different OLS estimates
...
- [Sample]{.hi-purple}: some portion of the population of interest to *represent the whole*
  - Samples examine part of a population to generate [statistics]{.hi-turquoise} used to [estimate]{.hi} population [parameters]{.hi-turquoise}
...
1. $\mathbb{E}[\hat{\beta_1}]$; what is the [center]{.hi-purple} of the distribution? 

2. $\sigma_{\hat{\beta_1}}$; how [precise]{.hi-purple} is our estimate? 
...
- This *is* a problem for **inference**, but we have a simple fix for this (more on this later)
...
- Autocorrelation won't impact estimates but rather inference
...
::: columns
::: {.column width="50%"}
- What makes one estimator (e.g. $\bar{H}$) better than another (e.g. $H_1$)?^[Technically, we also care about [consistency]{.hi-purple}: minimizing uncertainty about the correct value. The Law of Large Numbers (as the sample size grows ($n \rightarrow \infty$) the mean of the sample gets closer to the population avg), similar to CLT, permits this. We don't need to get too advanced about probability in this class.]
...
::: columns
::: {.column width="50%"}
- [Gauss-Markov Theorem]{.hi-turquoise}: The OLS estimator $\mathbf{\hat{\beta_1}}$ is the [Best (most effecient) Linear Unbiased Estimator (BLUE)]{.hi} estimator of  $\mathbf{\beta_1}$ **when $X$ is exogenous**
...
- Let's show that assuming exogeniety implies unbiased OLS estimates
...
- The true expected value of $\hat{\beta_1}$ is actually:^[We will prove this now]
