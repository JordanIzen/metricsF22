...
- Covariance is a common measure, but the units are meaningless, it is non-robust to changes of scale, and does not indicate how a change in one variable can be expected to change another
...
- Better to *standardize* covariance into a more intuitive concept which is robust to scale changes: [correlation, $r_{X,Y}$]{.hi} $\in [-1, 1]$
...
- Alternatively, take the average^[Over n-1, a *sample* statistic!] of the product of standardized $(Z$-scores for) each $(x_i,y_i)$ pair:
...
## Properties of Covarience and Correlation
- Let's discuss some properties of covarience and correlation on the board
...
$$\text{test score} = \beta_0 + \beta_1 \text{class size}+\text{factors}$$

- For now, we will ignore these "factors" until later

- Thus, $\beta_0 + \beta_1 \text{class size}$ gives the [average effect]{.hi-purple} of class sizes on scores

- Later, we will want to estimate the [marginal effect]{.hi} ([causal effect]{.hi}) $\beta_1$ of each factor on an individual district's test score, holding all other factors constant
:::
::: {.column width="50%"}
![](images/smallclass.jpg)
:::
...
- Known as a [linear regression model]{.hi} since it is [linear]{.hi-purple} in the parameters
  - i.e no parameter appears as exponent or is multiplied by another parameter 
...
- We can calculate $\hat{u_i}$ but notice that $\hat{u_i}$ does not enter our OLS regression line. Why? 
...
- Let's prove that these are the OLS Estimators on the board
...
## Regression Line as a CMF

- We can think of our regression line as a [conditional mean function (cmf)]{.hi}

- What is the expected value (mean) of $Y$?
$$\begin{align*}
\mathbb{E}(Y)&=\mathbb{E}(\beta_0 +\beta_1X + u) \\
&= \mathbb{E}(\beta_0) +\mathbb{E}(\beta_1 X) + \mathbb{E}(u) \\
&= \beta_0 + \beta_1 X +0 &&\text{Assume X is a fixed constant at given values} \\
\bar{Y} &= \beta_0 + \beta_1 X \\
\end{align*}$$

. . .

- Therefore, for a given value of $X$, the regression line tells us the average value of $Y$ (i.e $\mathbb{E}(Y_i | X_i)$)

. . .

- $\beta_0$ is the expected value of $Y$ when $X$ is zero; $\beta_1$ is the expected change in $Y$ when $X$ changes by one unit
...
This is `base R` (`tidymodels` is an alternative package that is a bit closer to `tidyverse`, but is a bit more complex to use) 
...
:::
::: {.column width="70%"}
- The `broom` package allows us to work with regression objects as tidier `tibbles` (as opposed to lists)
