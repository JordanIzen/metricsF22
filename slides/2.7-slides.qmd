...
# Digression: p-Values and the Philosophy of Science {.centered background-color="#314f4f"}

## Hypothesis Testing and the Philosophy of Science I

::: columns
::: {.column width="30%"}
![](images/rafisher.jpg){fig-align="center" width="400"}
...
[Fisher, R.A., 1931, *The Design of Experiments*]{.source}
...
::: columns
::: {.column width="60%"}
- Modern philosophy of science is largely based off of hypothesis testing and [falsifiability]{.hi-purple}, which form the "**Scientific Method**"^[Note: economics is a very different kind of "science" with a different methodology!]

- For something to be "scientific", it must be [falsifiable]{.hi-purple}, or at least [testable]{.hi-purple} (at least in principle)
...
- Rigorous course on statistics ([**ECMG 212**](https://ryansafner.com/courses/ecmg-212.html) or **MATH 112**) will spend weeks going through different types of tests:
...
![](images/onetest.png){fig-align="center"}
...
- [Test statistic $\delta$]{.hi-purple}: measures **how far what we observed in our sample $(\hat{\beta_1})$ is from what we would expect if the null hypothesis were true $(\beta_1=0)$**
  ...
  - In econometrics, we use $t$-distributions which have $n-k-1$ degrees of freedom^[Again, see last class's [appendix](/resources/appendices/2.6-appendix) for more on the t-distribution. $k$ is the number of independent variables our model has, in this case, with just one $X$, $k=1$. We use two degrees of freedom to calculate $\hat{\beta}_0$ and $\hat{\beta}_1$, hence we have $n-2$ df.]
...
![](images/tdistributionmeme.jpg){fig-align="center"}
...
- In `infer`'s language, `str` and `testscr` are `independent` $(\beta_1=0)$^[See more [here](https://moderndive.netlify.com/9-hypothesis-testing.html) about what other hypotheses you can test with `infer`]
...
- Compares to a .hi-purple[critical value] of $t^*$ (pre-determined by $\alpha$-level & $n-k-1$ df)
    - For 95% confidence, $\alpha=0.05$, $t^* \approx 2$^[Again, the **68-95-99.7%** empirical rule!]
...
![](images/chocolatefake.png){fig-align="center"}

[Source: [Washington Post](https://www.washingtonpost.com/news/morning-mix/wp/2015/05/28/how-and-why-a-journalist-tricked-news-outlets-into-thinking-chocolate-makes-you-thin/?hpid=z5)]{.source}
