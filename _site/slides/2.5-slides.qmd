...
1. [Goodness of fit of the model (SER)]{.hi-turquoise}^[<sup>.magenta[†]</sup> Recall from last class, the **S**tandard **E**rror of the **R**egression $\hat{\sigma_u} = \sqrt{\frac{\sum \hat{u_i}^2}{n-2}}$]
]
...
- Can use the `lmtest` package's function `bptest()`
    - $H_0$: homoskedastic^[More on hypothesis testing next week!]
...
- Heteroskedasticity is easy to fix with software that can calculate [robust standard errors]{.hi} (using the more complicated formula above)
