...
[4.3 --- Nonlinearity & Transformations]{.custom-title}
...
### [Standardizing & Comparing Across Units](#standardizing-comparing-across-units)
...
- OLS is commonly known as “[_linear_ regression]{.hi}” as it fits a **straight line** to data points
...
- Recall, [we care more](https://metricsf22.classes.ryansafner.com/slides/3.1-slides.html#/two-types-of-uses-for-regression) about accurately measuring the causal effect of $X \rightarrow Y$, rather than getting the most accurate prediction possible for $\hat{Y}$
...
- The [exponential function]{.red}, $Y=e^X$ or $Y=exp(X)$, where base $e=2.71828...$

- [Natural logarithm]{.blue} is the inverse, $Y=ln(X)$

## The Natural Logarithm: Review I {.smaller}

- [Exponents]{.hi} are defined as
...
- [**Example**]{.green}: $\color{#6A5ACD}{2}^{\color{#e64173}{3}}=\underbrace{\color{#6A5ACD}{2} \times \color{#6A5ACD}{2} \times \color{#6A5ACD}{2}}_{\color{#e64173}{n=3}}=\color{#314f4f}{8}$

. . .

- [Logarithms]{.hi-purple} are the inverse, defined as the exponents in the expressions above
...
- [**Example**]{.green}: $log_{\color{#6A5ACD}{2}}(\color{#314f4f}{8})=\color{#e64173}{3}$

## The Natural Logarithm: Review II

- Logarithms can have any base, but common to use the [natural logarithm]{.hi-purple} $(\ln)$ with base $\mathbf{e=2.71828...}$
...
# Standardizing & Comparing Across Units {.centered background-color="#314f4f"}
...
- An easy way is to [standardize]{.hi}^[Also called “centering” or “scaling.”] the variables (i.e. take the $Z$-score) 
...
## Comparing Coefficients of Different Units: Example {.smaller}
...
- [Marginal effects]{.hi-purple} on $Y$ (in *standard deviations* of $Y$) from 1 *standard deviation* change in $X$:
