---
title: "Problem Set 4"
author: "ECON 480 - Fall 2022"
date: "Due by Friday, November 11, 2022"
output:
  pdf_document:
...
Answer the following questions using `R`. When necessary, please write answers in the same document (knitted `qmd` to `html` or `pdf`, typed `.doc(x)`, or handwritten) as your answers to the above questions. Be sure to include (email or print an `.R` file, or show in your knitted `Quarto`) your code and the outputs of your code with the rest of your answers.

9. Download the `heightwages.csv` dataset. Download and read in `heightwages.csv` dataset. If you don't want to download/upload it, you can read it in directly from the url by running this chunk:
...
k. Make a regression table from part B and D using `modelsummary`.

\clearpage

10. Install and load the `wooldridge` package. This package contains datasets used in Jeffrey Wooldridge's *Introductory Econometrics: A Modern Approach* (the textbook that I used in *my* econometrics classes years ago!).
...
d. List *all* of the possible pathways from `cigs` to `bwght`. Which are “front-doors” and which are “back-doors?” Are any blocked by colliders? You can answer this using `dagitty.net,` and/or R.
e. What is the minimal sufficient set of variables we need to control in order to causally identify the effect of `cigs` on `bwght`? You can answer this using `dagitty.net,` and/or R.
f. Estimate the causal effect by running the appropriate regression in `R`. 
...
h. Perhaps our model is poorly specified. Maybe `motheduc` actually has a causal effect on `bwght`? Tweak your model on `dagitty.net` to add this potential relationship. What testable implications does this new model imply? You can answer this using `dagitty.net,` and/or R.
i. Test these implications appropriately in `R`, like you did in Part C. Does this model hold up well?
j. Under this new causal model, list *all* of the possible pathways from `cigs` to `bwght`. Which are “front-doors” and which are “back-doors?” Are any blocked by colliders? You can answer this using `dagitty.net,` and/or R.
k. Under this new causal model, what is the minimal sufficient set of variables we need to control in order to causally identify the effect of `cigs` on `bwght`? You can answer this using `dagitty.net,` and/or R.
